...
// ── Multi-tenant offline ID generation ────────────────────────────────────────

const DEVICE_ID_KEY = 'blendpos_device_id';

/**
 * Returns a stable device identifier for this browser/terminal.
...
/**
 * Generates a globally unique offline_id without server coordination.
 * Format: {tenantId}:{deviceId}:{timestamp_ms}:{random_hex_4}
 *
 * Collision probability: (1/65536) per millisecond per device — negligible.
 * The backend enforces UNIQUE (tenant_id, offline_id) as a final safety net.
 */
export function generateOfflineId(tenantId: string): string {
