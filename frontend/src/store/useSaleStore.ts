...
import { enqueueSale, generateOfflineId, trySyncQueue } from '../offline/sync';
    ...
    persist(
        (set, get) => ({
            ...
            confirmSale: (pago) => {
                ...
                // Generate a globally-unique offline_id scoped to tenant + device.
                // Falls back to plain UUID if tenantId is not yet available (demo mode).
                const tenantId = useAuthStore.getState().tenantId;
                const offlineId = tenantId
                    ? generateOfflineId(tenantId)
                    : crypto.randomUUID();
