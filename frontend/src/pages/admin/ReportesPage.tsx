...
export function ReportesPage() {
    ...
    const exportCSV = useCallback((type: 'ventas' | 'productos' | 'medios' | 'cajeros') => {
        ...
        const sep = ';';
        if (type === 'ventas') {
            csv = `Fecha${sep}Ventas\n` + ventasPeriodo.map((v) => `${v.fecha}${sep}${v.total}`).join('\n');
        } else if (type === 'productos') {
            csv = `Producto${sep}Cantidad${sep}Total\n` + topProductos.map((p) => `${p.nombre}${sep}${p.cantidad}${sep}${p.total}`).join('\n');
        } else if (type === 'medios') {
            csv = `Método${sep}Cantidad${sep}Total${sep}Porcentaje\n` + mediosPago.map((m) => `${m.metodo}${sep}${m.cantidad}${sep}${m.total}${sep}${(m.total / (totalMediosPago || 1) * 100).toFixed(1)}%`).join('\n');
        } else if (type === 'cajeros') {
            csv = `Cajero${sep}Ventas${sep}Total${sep}Ticket Promedio\n` + cajeros.map((c) => `${c.cajero_nombre}${sep}${c.cantidad_ventas}${sep}${c.total_ventas}${sep}${c.ticket_promedio}`).join('\n');
        }
