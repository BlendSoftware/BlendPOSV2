...
class AFIPClient:
    ...
    def __init__(
        ...
        Modo legacy: pasar cert_path + key_path con rutas a archivos en disco.
        Modo stateless (F1-4): pasar cert_pem + key_pem con el contenido PEM directo.
          El cliente escribe los PEMs a un directorio temporal, y los elimina al llamar
          cleanup() o al destruirse el objeto (multi-tenant, sin estado compartido).

        Args:
            cuit_emisor: CUIT del emisor (sin guiones)
            cert_path: Ruta al certificado .crt (modo legacy)
            key_path: Ruta a la clave privada .key (modo legacy)
            ...
            cert_pem: Contenido PEM del certificado (modo stateless — F1-4)
            key_pem: Contenido PEM de la clave privada (modo stateless — F1-4)
        """
        ...
        # Stateless mode: write PEM content to a private temp dir
        self._temp_dir: Optional[str] = None
        if cert_pem and key_pem:
            self._temp_dir = tempfile.mkdtemp(prefix="afip_certs_")
            self.cert_path = os.path.join(self._temp_dir, "cert.crt")
            self.key_path = os.path.join(self._temp_dir, "cert.key")
            try:
    ...
    # Keys are per-CUIT and per-mode so multi-tenant stateless requests don't
    # overwrite each other's cached tokens (F1-4).

    _REDIS_TTL_SEC = 43200  # 12 hours — matches AFIP token lifetime

    @property
    def _redis_token_key(self) -> str:
        mode = "homo" if self.homologacion else "prod"
        return f"afip:wsaa:{self.cuit_emisor}:{mode}:token"

    @property
    def _redis_sign_key(self) -> str:
        mode = "homo" if self.homologacion else "prod"
        return f"afip:wsaa:{self.cuit_emisor}:{mode}:sign"

    @property
    def _redis_exp_key(self) -> str:
        mode = "homo" if self.homologacion else "prod"
        return f"afip:wsaa:{self.cuit_emisor}:{mode}:exp"

    def _save_token_to_cache(self) -> None:
        """Persist the current WSAA token+sign in Redis (keyed per CUIT+mode)."""
        ...
        try:
            ...
            pipe.execute()
            logger.debug("Token WSAA guardado en Redis — key: %s (TTL %ds)", self._redis_token_key, self._REDIS_TTL_SEC)
    ...
    def _load_token_from_cache(self) -> bool:
        """
        Try to restore a previously issued WSAA token from Redis (keyed per CUIT+mode).
    ...
    def cleanup(self) -> None:
        """
        Elimina los archivos de certificado temporales (modo stateless — F1-4).
        Llamar explícitamente en un bloque finally tras cada request stateless.
        """
        if self._temp_dir and os.path.exists(self._temp_dir):
            try:
                shutil.rmtree(self._temp_dir, ignore_errors=True)
                logger.debug("Directorio temporal de certificados eliminado: %s", self._temp_dir)
            except Exception as e:
                logger.warning("No se pudo eliminar directorio temporal: %s", e)
            finally:
                self._temp_dir = None

    def __del__(self) -> None:
        """Fallback cleanup si cleanup() no fue llamado explícitamente."""
        self.cleanup()
