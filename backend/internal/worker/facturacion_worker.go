...
func (w *FacturacionWorker) buildAFIPPayload(ctx context.Context, venta *model.Venta, payload *FacturacionJobPayload) infra.AFIPPayload {
	...
	// Use ObtenerConfiguracionCompleta to retrieve CUIT, PdV, condicion fiscal,
	// and PEM certs for stateless sidecar mode (F1-4).
	certPEM := ""
	keyPEM := ""
	modo := ""
	if w.configFiscalSvc != nil {
		if fiscalCfg, err := w.configFiscalSvc.ObtenerConfiguracionCompleta(ctx); err == nil && fiscalCfg != nil && fiscalCfg.CUITEmsior != "" {
	...
	return infra.AFIPPayload{
		...
		// F1-4: PEM certs for stateless multi-tenant sidecar mode
		CertPEM: certPEM,
		KeyPEM:  keyPEM,
		Modo:    modo,
	}
