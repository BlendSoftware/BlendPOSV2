...
func processRetries(ctx context.Context, cfg RetryCronConfig) {
	...
	for i := range comprobantes {
		...
		// Read fiscal config including PEM certs for stateless sidecar mode (F1-4)
		cuitEmisor := ""
		...
		if cfg.ConfigFiscalSvc != nil {
			if fiscalCfg, err := cfg.ConfigFiscalSvc.ObtenerConfiguracionCompleta(ctx); err == nil && fiscalCfg != nil {
