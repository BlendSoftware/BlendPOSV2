...
type AFIPPayload struct {
	...
	// Multi-tenant stateless mode (F1-4): PEM content sent per-request.
	// When set, the sidecar creates an ephemeral AFIPClient for this request
	// instead of using its global singleton.
	CertPEM string `json:"cert_pem,omitempty"` // Certificado PEM (contenido, no ruta)
	KeyPEM  string `json:"key_pem,omitempty"`  // Clave privada PEM (contenido, no ruta)
	Modo    string `json:"modo,omitempty"`     // "homologacion" | "produccion"
}
