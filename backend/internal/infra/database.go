...
// NewDatabaseReadReplica opens a GORM connection to the PostgreSQL read replica.
// If replicaURL is empty or the connection fails, it falls back to primaryDB
// transparently — callers are unaffected (F1-9).
//
// Note: the replica does NOT run migrations or schema patches; those are the
// primary's responsibility. The replica should already be in sync via streaming
// replication or RDS read replica.
func NewDatabaseReadReplica(primaryDB *gorm.DB, replicaURL string) *gorm.DB {
	if replicaURL == "" {
		log.Warn().Msg("DATABASE_READ_REPLICA_URL not set — analytics queries will use the primary DB")
		return primaryDB
	}

	db, err := gorm.Open(postgres.Open(replicaURL), &gorm.Config{
		Logger: logger.Default.LogMode(logger.Silent),
	})
	if err != nil {
		log.Warn().Err(err).Msg("read replica connection failed — falling back to primary DB")
		return primaryDB
	}

	sqlDB, err := db.DB()
	if err != nil || sqlDB.Ping() != nil {
		log.Warn().Msg("read replica ping failed — falling back to primary DB")
		return primaryDB
	}

	// Read replica uses a smaller pool: analytics queries are read-only and
	// lower-priority than transactional writes on the primary.
	sqlDB.SetMaxOpenConns(10)
	sqlDB.SetMaxIdleConns(5)
	sqlDB.SetConnMaxLifetime(5 * time.Minute)

	log.Info().Str("url", replicaURL).Msg("read replica connected")
	return db
}
...
func applySchemaPatches(db *gorm.DB) error {
	patches := []string{
		...
		// migration 000015: compras and compra_items tables
